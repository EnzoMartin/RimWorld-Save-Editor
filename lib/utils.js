...
module.exports = {
  ...
  isColonist(thing){
    return (
      this.isHuman(thing)
            && thing.faction.indexOf(config.colonyFaction) !== -1
    );
  },
  /**
  ...
  getColonyFaction: (factions) => {
    ...
    return factions.find((faction) => {
      return faction.loadID && faction.loadID[0] == config.colonyFactionId;
    });
  },
  /**
  ...
  findPlayerColony:(game) => {
    ...
    try {
      ...
      const index = game.world[0].factionManager[0].allFactions[0].li.find((faction) => {
        return faction.def[0] === def;
      }).loadID[0];
      faction = `Faction_${index}`;
    } catch (err){
